  ...
  finishEditHandler = postData => {
    ...
    fetch('http://localhost:8080/post-image', {
      ...
      let graphqlQuery = {
        query: `
    mutation{
          ${this.state.editPost ? 'updatePost' : 'createPost'}(${this.state.editPost ? `postId: "${this.state.editPost._id}",` : ''}postInput: {title: "${postData.title}", content: "${postData.content}", imageUrl: "${imageUrl}"
      }){
      _id
      title
      content
